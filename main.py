# Инстаграм постоянно переписывается, поэтому многие функции могут не работать.
# Придётся заново обновлять Xpath для кнопок на сайте инстаграмма
...
class InstagramBot():
    ...
    def get_all_followers(self, userpage):
        ...
        if self.xpath_exists(wrong_userpage):
            ...
            followers_button = browser.find_element_by_xpath("//section/main/div/header/section/ul/li[2]/a/span")
            ...
            followers_ul = browser.find_element_by_xpath("//div[4]/div/div/div[2]")
                ...
                with open(f"{file_name}/{file_name}.txt") as text_file:
                    ...
                    for user in users_urls[0:10]:
                        try:
                            ...
                            if self.xpath_exists("//section/main/div/header/section/div[1]/div/a"):

                                print("Это наш профиль, уже подписан, пропускаем итерацию!")
                            elif self.xpath_exists("//section/main/div/header/section/div[1]/div[2]/div/span/span[1]/button/div/span"):
                                print(f"Уже подписаны, на {page_owner} пропускаем итерацию!")
                            else:
                                time.sleep(random.randrange(4, 8))

                                if self.xpath_exists("//section/main/div/div/article/div[1]/div/h2"):
                                    try:
                                        follow_button = browser.find_element_by_xpath("//section/main/div/header/section/div[1]/div[1]/button").click()
                                        print(f'Запросили подписку на пользователя {page_owner}. Закрытый аккаунт!')
                                    except Exception as ex:
                                        print(ex)
                                else:
                                    try:
                                        if self.xpath_exists("//section/main/div/header/section/div[1]/div[1]/button"):
                                            follow_button = browser.find_element_by_xpath("//section/main/div/header/section/div[1]/div[1]/button").click()
                                            print(f'Подписались на пользователя {page_owner}. Открытый аккаунт!')
                                        else:
                                            follow_button = browser.find_element_by_xpath("//section/main/div/header/section/div[1]/div[1]/div/span/span[1]/button").click()
                                            print(f'Подписались на пользователя {page_owner}. Открытый аккаунт!')
                                    except Exception as ex:
                                        print(ex)
    ...
    def send_direct_message(self, usernames="", message="", img_path=''):
        ...
        direct_message_button = "//section/nav/div[2]/div/div/div[3]/div/div[2]/a"
        ...
        if self.xpath_exists("/html/body/div[4]/div/div"):
            browser.find_element_by_xpath("//div[4]/div/div/div/div[3]/button[2]").click()
        time.sleep(random.randrange(2, 4))

        send_message_button = browser.find_element_by_xpath("//section/div/div[2]/div/div/div[2]/div/button").click()
        time.sleep(random.randrange(2, 4))

        # отправка сообщения нескольким пользователям
        for user in usernames:
            # вводим получателя
            to_input = browser.find_element_by_xpath("//div[4]/div/div/div[2]/div[1]/div/div[2]/input")
            to_input.send_keys(user)
            time.sleep(random.randrange(2, 4))

            # выбираем получателя из списка
            users_list = browser.find_element_by_xpath("//div[4]/div/div/div[2]/div[2]").find_element_by_tag_name("button").click()
            time.sleep(random.randrange(2, 4))

        next_button = browser.find_element_by_xpath("//div[4]/div/div/div[1]/div/div[2]/div/button").click()
        time.sleep(random.randrange(2, 4))

        # отправка текстового сообщения
        if message:
            text_message_area = browser.find_element_by_xpath("//section/div/div[2]/div/div/div[2]/div[2]/div/div[2]/div/div/div[2]/textarea")
        ...
        if img_path:
            send_img_input = browser.find_element_by_xpath("//section/div/div[2]/div/div/div[2]/div[2]/div/div[2]/div/div/form/input")
    ...
    def unsubscribe_for_all_users(self, userpage):
        ...
        following_button = browser.find_element_by_xpath("//section/main/div/header/section/ul/li[3]/a")
        ...
        for loop in range(1, loops_count + 1):
            ...
            following_div_block = browser.find_element_by_xpath("//div[4]/div/div/div[2]/ul/div")
            ...
            for user in following_users:
                ...
                time.sleep(random.randrange(3, 6))
                unfollow_button = browser.find_element_by_xpath("//div[5]/div/div/div/div[3]/button[1]").click()
    ...
    def smart_unsubscribe(self, username):
        ...
        following_ul = browser.find_element_by_xpath("//div[4]/div/div/div[2]")
        ...
        try:
            ...
            try:
                ...
                for user_url in unfollow_users_list:
                    ...
                    # кнопка отписки
                    unfollow_button = browser.find_element_by_xpath("//section/main/div/header/section/div[1]/div[1]/div/div[2]/div/span/span[1]/button")
                    unfollow_button.click()

                    time.sleep(random.randrange(4, 6))

                    # подтверждение отписки
                    unfollow_button_confirm = browser.find_element_by_xpath("//div[4]/div/div/div/div[3]/button[1]")
...
# my_bot.put_exactly_like("https://www.instagram.com/p/userpost/")
# my_bot.get_all_posts_urls("https://www.instagram.com/username/")
# my_bot.get_all_followers('https://www.instagram.com/username/')
# my_bot.download_userpage_image("https://www.instagram.com/username/")
